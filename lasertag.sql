...
CREATE FUNCTION notify_table_trigger() RETURNS trigger AS $$ DECLARE BEGIN PERFORM pg_notify('watchers', jsonb_merge(get_takedown_value(player_frequency), get_hit_value(player_frequency))); RETURN NULL; END; $$ LANGUAGE plpgsql;

-- Create the function to be used by the triggers on the takedown view (announces a 'watcher' for NodeJS socekt)
--CREATE FUNCTION notify_takedowns_view_trigger() RETURNS trigger AS $$ DECLARE BEGIN PERFORM pg_notify('watchers', get_takedown_value(NEW.player_frequency)::text); RETURN new; END; $$ LANGUAGE plpgsql;

-- Create the function to be used by the triggers on the hits view (announces a 'watcher' for NodeJS socekt)
--CREATE FUNCTION notify_hits_view_trigger() RETURNS trigger AS $$ DECLARE BEGIN PERFORM pg_notify('watchers', get_hit_value(NEW.player_frequency)::text); RETURN new; END; $$ LANGUAGE plpgsql;
